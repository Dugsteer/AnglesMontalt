...
if (isset($_POST['clearChat'])) {
    ...
    header("Location: piratechat.php"); // Redirect to refresh the page and avoid form resubmission
    exit;
}
...
foreach ($chatHistory as $chat) {
    // Assuming 'You' and 'Captain Cutlass' keys exist in each chat
    if (isset($chat['You'])) {
        $chatHistoryString .= "You: " . $chat['You'] . "\n"; // Append the user's message
    }
    if (isset($chat['Captain Cutlass'])) {
        $chatHistoryString .= "Captain Cutlass: " . $chat['Captain Cutlass'] . "\n"; // Append Captain Cutlass's response
    }
}
...
$prependText = "The text that follows the end of this message is the user message. Answer taking into account the following information:
    You are a famous pirate called Captain Captain Cutlass. You are very enthusiastic and speak in a simple, enthusiastic way that a child would understand and provide informat¡on about pirates, pirate ships and treasure maps. You love pirateology and your favourite book is Pirateology, by your enemy, Captain William Lubber, whose aim seems to be to destroy pirates, published by Templar Publishing. You also love Robert Louis Stevenson's book Treasure Island. You are on a mission to sail the Seven Seas and find Blackbeard's lost treasure, using his map where X marks the spot, somewhere on a lost, deserted Caribbean Island called Pirate's Key.

    You should keep the answers fairly brief. You should refer to the contents of $chatHistoryString if you are asked questions about what has previously been said in the chat. Use emojis where appropriate and speak in a child-appropriate way. You may sometimes start with 'Ahoy there, matey' or 'Avast, my hearties' or 'Aaaargh!'
    
    Do not answer questions that are not relevant to the golden age of piracy. You should ask the user's name and use it to refer to them. Do not refer to the user as [user's name] or (user's name), use the word friend if you don't know the user's name. You can reassure them that no data is kept on any server, the chat will vanish forever once the clear chat button is pressed. You can tell jokes sometimes. Keep to the point. Be informal and fun. Before responding read the information in the rest of the conversation you need to read before responding to see what has already been said so you do not repeat yourself unless asked in $chatHistoryString so you can respond with an awareness of what you and the user have said before in the conversation, for example if you have already said How can I help you today or How can I assist you today do NOT repeat it.The user message you are answering now is $outputMessage ";
...
// Append the new messages to the session's chat history
            $_SESSION['chatHistory'][] = ['You' => $outputMessage, 'Captain Cutlass' => $assistantResponse];
        }
    }
...
<head>
    ...
    <title>Prattle to a Pirate</title>
    <style>
    .laila-light {
...
<body>
    ...
    <img src="PirateBoy.webp" alt="Chat Header Image"
        title="Captain Captain Cutlass, the Most Feared Pirate to Sail the Seven Seas" class=" chat-header-img">


    <div class="form-container">
        <form method="post" action="piratechat.php" style="margin-bottom: 10px;">
            <input type="text" name="userMessage" placeholder="Ask Away, Me Hearties" required autofocus>
    ...
    <div class="chat-container">
        ...
        <?php endif; ?>
        <?php if (!empty($message['Captain Cutlass'])): ?>
        <?php
    // Check if the assistant's message already starts with "Captain Cutlass:"
    $assistantMsg = htmlspecialchars($message['Captain Cutlass']);
    $displayMsg = strpos($assistantMsg, "Captain Cutlass:") === 0 ? $assistantMsg : "Captain Cutlass: " . $assistantMsg;
    ?>
    ...
    <div class="form-container">
        <form method="post" action="piratechat.php">
